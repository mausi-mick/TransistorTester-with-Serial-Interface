...
void DiodeSymbol_ApinCpin(uint8_t nn) {
  ...
  Serial.print("1  "); // diode
  if      (diodes.Anode[nn] == 0 and diodes.Cathode[nn] == 1)Serial.println("AK ");
  else if (diodes.Anode[nn] == 0 and diodes.Cathode[nn] == 2)Serial.println("A K");
  else if (diodes.Anode[nn] == 1 and diodes.Cathode[nn] == 2)Serial.println(" AK");
  DiodeSymbol_ACpin(nn);
}
void DiodeSymbol_ACpin(uint8_t nn) {
  lcd_MEM_string(AnKat_str);	// "->|-"
  lcd_testpin(diodes.Cathode[nn]);
//  Serial.println(1+diodes.Cathode[nn]); 
}
void DiodeSymbol_CpinApin(uint8_t nn) {
  lcd_testpin(diodes.Cathode[nn]);
  Serial.print("1  "); // diode
  if      (diodes.Cathode[nn] == 0 and diodes.Anode[nn] == 1)Serial.println("KA ");
  else if (diodes.Cathode[nn] == 0 and diodes.Anode[nn] == 2)Serial.println("K A");
  else if (diodes.Cathode[nn] == 1 and diodes.Anode[nn] == 2)Serial.println(" KA");
  DiodeSymbol_CApin(nn);
}
void DiodeSymbol_CApin(uint8_t nn) {
  lcd_MEM_string(KatAn_str);	// "-|<-"
  lcd_testpin(diodes.Anode[nn]);
 // Serial.println(1+diodes.Anode[nn]); 
}
