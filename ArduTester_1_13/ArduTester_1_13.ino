// new: device infos over RX/TX (9600 Baud):
// 1.sign: device id: 1=Diode, 2= transistor(bip.), 3= Mosfet, 4=J-Fet, 5=IGBT
// 2.sign: polarity : N / P for Trans,Mosfet,J-Fet,IBBT?
// 3.sign: Mode:      E / D E=enhancement, D=Depletion (Mosfet: E/D J-Fet=D, IGBT ?
// 4.,5,6: Pin-Name : Diode: K,A,_ Trans.: E,B,C, j-/Mos-Fet: S,G,D, IGBT: E,G,C
//#######################################################################
...
//       _trans = &ntrans;  is allready selected a default
    ...
    Serial.begin(9600);
    delay(400);
    Serial.print("2");                   // 2=transistor
    if(PartMode==12) Serial.print("N "); // npn
    else             Serial.print("P "); // pnp
    delay(300);
    if(PartMode == PART_MODE_NPN) {
//       _trans = &ntrans;  is allready selected a default
       lcd_MEM_string(NPN_str);		//"NPN "
...
//       _trans = &ntrans;	is allready selected as default
    ...
    if (part_code == PART_MODE_JFET) {
       lcd_MEM_string(jfet_str);	//"-JFET"
       Serial.print("4");   // JFET
       Serial.print(n_p);   // n or p
       Serial.print("D"); // j-fet: depletion
       delay(200);
    } else {		// no JFET
     
       if ((PartMode&D_MODE) == D_MODE) {
          lcd_data('D');			// N-D or P-D
          Serial.print("3");  // 3=MOS-Fet
          Serial.print(n_p);  // n or p
          Serial.print("D");  // depletion
          delay(100);
       } else {
          lcd_data('E');      // N-E or P-E
          Serial.print("3");  // 3=MOS-Fet
    ...
    if (part_code == PART_MODE_IGBT) {         // 3
       PinLayout('E','G','C'); 		//  EGC= or 123=...
     //  Serial.println(" EGC");delay(300);
    } else if (part_code == PART_MODE_JFET) {  // 4
       PinLayout('?','G','?'); 		//  ?G?= or 123=...
     //  Serial.println(" ?G?");delay(300);
    } else {                      //  2
       PinLayout('S','G','D'); 		//  SGD= or 123=...
    //   Serial.println(" SGD");delay(300);
    }
#endif  /* WITH_GRAPHICS */
